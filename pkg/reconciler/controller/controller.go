...
func NewController(
	...
	r := &Reconciler{
		...
		rocketmqClientSet:       rocketmqChannelClient.Get(ctx),
	...
	if err := envconfig.Process("", env); err != nil {
		logging.FromContext(ctx).Sugar().Panicf("unable to process Rocketmq channel's required environment variables: %v", err)
	}

	if env.Image == "" {
		logging.FromContext(ctx).Panic("unable to process Rocketmq channel's required environment variables (missing DISPATCHER_IMAGE)")
	}
	...
	if _, err := kubeclient.Get(ctx).CoreV1().ConfigMaps(system.Namespace()).Get("config-rocketmq", metav1.GetOptions{}); err == nil {
		...
		logging.FromContext(ctx).With(zap.Error(err)).Fatal("Error reading ConfigMap 'config-rocketmq'")
	}

	logging.FromContext(ctx).Info("Setting up event handlers")
	rocketmqChannelInformer.Informer().AddEventHandler(controller.HandleAll(impl.Enqueue))
	...
	// Call GlobalResync on rocketmqchannels.
	grCh := func(obj interface{}) {
		impl.GlobalResync(rocketmqChannelInformer.Informer())
	}
